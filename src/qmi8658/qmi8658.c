...
void qmi8658_process_raw()
{
  ...
  double g_fs = (double) qmi_state.imu_config.accl_config.imu_settings.g_config;
  double dps_fs = (double) qmi_state.imu_config.accl_config.imu_settings.dps_config;
  accl->x = ((double)accl->raw[0]) * (g_fs / 32768.0);
  accl->y = ((double)accl->raw[1]) * (g_fs / 32768.0);
  accl->z = ((double)accl->raw[2]) * (g_fs / 32768.0);
  gyro->x = ((double)gyro->raw[0]) * (dps_fs / 32768.0);
  gyro->y = ((double)gyro->raw[1]) * (dps_fs / 32768.0);
  gyro->z = ((double)gyro->raw[2]) * (dps_fs / 32768.0);
  gprintf(DEBUG, "a: [ %2.3lf %2.3lf %2.3lf ] g: [ %4.3lf, %4.3lf, %4.3lf ]\033[K\r", accl->x,
      accl->y, accl->z, gyro->x, gyro->y, gyro->z);
}
...
{
  gprintf(DEBUG, "a: [%5d, %5d, %5d] g: [%5d, %5d, %5d], ts: %10d\r", qmi_state.accl.raw[0],
      qmi_state.accl.raw[1], qmi_state.accl.raw[2], qmi_state.gyro.raw[0], qmi_state.gyro.raw[1],
      qmi_state.gyro.raw[2], qmi_state.imu_timestamp);
}
